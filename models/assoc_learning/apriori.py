...
def inspect(data):
    ...
    confidences = [result[2][0][2] for result in data]
    lifts = [result[2][0][3] for result in data]
...
results = list(rules)
resultsAsDataframe = pd.DataFrame(inspect(results),columns=['lhs','rhs','support','confidence','lift'])
print(resultsAsDataframe)